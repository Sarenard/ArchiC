...
class Executable():
    def __init__(self):
        self.data = dict()     # maps addresses to `encode()`able objects
    ...
    def add_label(self, name):
        ...
        try:
            addr = max(self.data) + 4
        except ValueError: # happens when data is still empty
            addr = 0
    ...
    def append(self, obj):
        try:
            ...
            addr = 0
        self.data[addr] = obj
        obj.addr = addr # so that `obj` knows its address
